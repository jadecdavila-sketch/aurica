export interface ProblemDescription {
  /** Matches the lobe's problemId on the home page. */
  id: string;
  /** The lobe title, echoed at the top of the drawer. */
  title: string;
  /** Body paragraphs. `**bold**` markers render as emphasis. */
  paragraphs: string[];
}

/**
 * The "what problem this solves" copy for the three home-page lobes, shown in
 * the ProblemSheet drawer. Source: docs/homepage_problem_descriptions.md
 * (council-voice warm). The docs file lives outside src/, so the build cannot
 * reach it - keep this file in sync by hand if the source copy changes.
 */
export const problemDescriptions: ProblemDescription[] = [
  {
    id: 'north-star',
...
export const getProblemDescription = (
  id: string,
): ProblemDescription | undefined => problemDescriptions.find((p) => p.id === id);
